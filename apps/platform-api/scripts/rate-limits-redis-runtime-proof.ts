...
type RlDeps = {
  rateLimits: RedisRateLimitRepository;
  entitlements: PostgresEntitlementRepository;
  audit: AuditEventPort;
};

async function seedOrgsAndPolicies(
  su: pg.Pool,
  entitlements: PostgresEntitlementRepository,
  deps: RlDeps
): Promise<{ orgA: string; orgB: string }> {
...
async function checkAllowDenyAndIsolation(
  redis: ReturnType<typeof createRedisClient>,
  redisRepo: RedisRateLimitRepository,
  deps: RlDeps,
  orgA: string,
  orgB: string,
  keyTouched: string[]
  ...
  check("readiness reports ready when Redis is reachable", ready.status === "ready", ready.detail);
...
async function checkFixedWindowReset(
  redis: ReturnType<typeof createRedisClient>,
  redisRepo: RedisRateLimitRepository,
  durable: PostgresRateLimitRepository,
  orgA: string,
  keyTouched: string[]
): Promise<void> {
...
async function checkDegradedFallback(
  durable: PostgresRateLimitRepository,
  orgB: string
): Promise<void> {
...
async function main(): Promise<void> {
  ...
  try {
    const seeded = await seedOrgsAndPolicies(su, entitlements, deps);
    orgA = seeded.orgA;
    orgB = seeded.orgB;

    await checkAllowDenyAndIsolation(redis, redisRepo, deps, orgA, orgB, keyTouched);
    await checkFixedWindowReset(redis, redisRepo, durable, orgA, keyTouched);
    await checkDegradedFallback(durable, orgB);
  } catch (err) {
