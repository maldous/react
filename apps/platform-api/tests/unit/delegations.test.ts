...
describe("delegations usecase", () => {
  ...
  it("delegateGrant: audit-before-mutation (ADR-ACT-0154); server-side stamps grantedBy from ctx.userId regardless of input.grantedBy", async () => {
    ...
    // ADR-ACT-0154: audit MUST happen BEFORE mutation. Vitest invocation
    // orderings guarantee the temporal sequence of mock calls.
    const auditOrder = (a.audit.emit as Mock).mock.invocationCallOrder[0] ?? 0;
    const grantOrder = (a.port.grantDelegation as Mock).mock.invocationCallOrder[0] ?? 0;
    expect(auditOrder).toBeGreaterThan(0);
    expect(grantOrder).toBeGreaterThan(0);
    expect(auditOrder).toBeLessThan(grantOrder);

    // Pre-mutation audit envelope carries the intent only — no delegationId
    // because the id is generated by the port AFTER audit succeeds.
    expect(a.audit.emit).toHaveBeenCalledWith({
      action: "Delegation.Granted",
      actorId: ctx.userId,
      organisationId: ORG,
    });
    expect(a.audit.emit).not.toHaveBeenCalledWith(
      expect.objectContaining({ delegationId: expect.anything() })
    );

    // Mutation uses the server-side-stamped input.
