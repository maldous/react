...
export function makeDelegationsUseCases(deps: DelegationsDeps) {
  ...
  return {
    ...
    async delegateGrant(
      ...
      // ADR-ACT-0154 invariant: audit BEFORE mutation. The intent envelope
      // (no delegationId yet — id is generated by the port) is the audit
      // subject. If the audit backend fails, the row never lands.
      await audit.emit({
