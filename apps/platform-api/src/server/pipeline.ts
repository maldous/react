...
import { httpRequestsTotal, httpRequestDurationSeconds } from "../adapters/prometheus-metrics.ts";
...
export function createRouter(
  ...
  const coreHandler = async (
    ...
    try {
      ...
      httpRequestDurationSeconds.observe({ method, route: matchingRoute.path }, durationMs / 1000);
    } catch (err) {
      const durationMs = Number(process.hrtime.bigint() - startedAt) / 1_000_000;
