...
import { test, expect, E2E_STAGE, scenario } from "../support/correlation.ts";
...
test.describe("persona-matrix — authed link/route/API crawl per persona", () => {
  ...
  for (const persona of personas) {
    test(
      `${persona.personaId} (${persona.authMode})`,
      // Canonical scenario id (ADR-ACT-0285 closure) — explicit per-persona id from the
      // controlled persona-registry expansion declared in e2e/scenario-manifest.json.
      // Deterministic and validated; never a sanitised test title.
      { annotation: scenario(`persona-matrix:${persona.personaId}`) },
      async ({ page, baseURL }, testInfo) => {
        ...
        if (persona.authMode === "real" && username) {
          if (persona.personaId.includes("disabled")) {
            ...
            await page
              .waitForURL(new RegExp(`^${escaped}/?$`), { timeout: 20_000 })
              .catch(() => {});
            authed = true;
          }
        }
