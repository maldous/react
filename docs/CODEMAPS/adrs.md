...
76 Architecture Decision Records (ADR-0001 through ADR-0076). ADR-0018 is intentionally absent. ADR-0001 through ADR-0052 are **Accepted**. Of the Universal Service Foundation set: ADR-0053, ADR-0054, ADR-0055, ADR-0056, ADR-0058 (ADR-ACT-0253/0254) and ADR-0061, ADR-0067 (ADR-ACT-0256, Phase 2 metering/quota), ADR-0065 (ADR-ACT-0257, Phase 3 developer-platform foundation — SDK/portal-gateway/sandbox/full-drift remain Proposed sub-decisions), ADR-0060 (ADR-ACT-0258, Phase 4 search — built-in Postgres FTS; composed engine remains a Proposed Phase-4.5 sub-decision), and ADR-0059 (ADR-ACT-0259, Phase 5 event bus + durable workers + DLQ/redrive — workflow engine + composed bus remain Proposed sub-decisions; notifications split to ADR-0068), and ADR-0068 (ADR-ACT-0260, Phase 6 end-user profile self-service + notification preferences + local notification substrate — composed providers + real delivery transports remain Proposed Phase-6.5 sub-decisions), and ADR-0062 (ADR-ACT-0261, Phase 7 built-in observability/alerting/incident foundation — composed Prometheus/Tempo/Alertmanager + on-call + status page remain Proposed Phase-7.5 sub-decisions), and ADR-0069 (ADR-ACT-0265, Tier-1 kernel runtime secrets — built-in Postgres secret store + composed OpenBao provider, both live-proven; production sealed/HA topology + rotation automation remain Proposed sub-decisions), and ADR-0070 (ADR-ACT-0266, Tier-1 kernel provider configuration plane — capability→provider binding per environment with credentials by secretRef + adapter-confirmed readiness; per-tenant overrides + auto-reconciliation remain Proposed sub-decisions), and ADR-0071 (ADR-ACT-0271, composed-provider readiness — generic readiness probe + Meilisearch/Prometheus/Tempo/Alertmanager compose profiles, live readiness-proven; backend integration + Windmill/Temporal compose remain Proposed sub-decisions), and ADR-0072 (ADR-ACT-0274, environment registry + Makefile-driven bootstrap + controlled operations — hand-maintained env-file removal, environment registry, operations port, and global admin handoff, all live/deterministically proven), and ADR-0073 (ADR-ACT-0275, composed-service SSO via Keycloak OIDC — opt-in OIDC clients + realm-role mappers for Grafana/SonarQube/MinIO/pgAdmin, statically validated; live click-through proof deferred to a running stack), and ADR-0074 (ADR-ACT-0277, browser-side diagnostics via Grafana Faro — Faro Web SDK → same-origin `/faro/collect` → Alloy faro.receiver → Loki; statically validated via vite build/alloy fmt/caddy validate, live browser proof deferred), and ADR-0063 (ADR-ACT-0267, data governance — hardened + split into six sub-decisions: catalog/classification/DSR/import-export/access-reviews/compliance; no capability delivered), ADR-0064 (ADR-ACT-0268, backup/recovery/retention/legal-hold — hardened; only proof:backup-local delivered; offsite/PITR/legal-hold/restore-drill remain Proposed sub-decisions), and ADR-0066 (ADR-ACT-0269, support/break-glass — hardened; only the audited support-session usecase delivered; host-origin escalation remains an unresolved policy gate), and ADR-0075 (ADR-ACT-0285, UI/UX E2E contract + environment-appropriate confidence ladder — three machine-readable e2e/ registries + coverage/persona/ui-contract validators wired into make all; Phase 1 spine delivered, deeper execution phased), and ADR-0076 (ADR-ACT-0293, typed composition-root configuration foundation — config slice 1: typed kernel + PlatformApiConfig/WebPublicConfig + validation/immutability/test-seam/classification; V1C-CONF-04/06 remain) are **Accepted**. ADR-0057 has been **re-scoped to billing/invoicing/payment** (Phase 9, hardened to a provider decision — Kill Bill primary, Lago pending license, OpenMeter rejected — but still Proposed/undelivered; entitlements→0058, metering/quota→0067). ADR-0057 is the only ADR remaining **Proposed**.
...
| ADR-0071 | Composed provider readiness                                  | 2026-06-13 | 0071-composed-provider-readiness.md                           |
| ADR-0072 | Environment registry, bootstrap & controlled operations      | 2026-06-14 | 0072-environment-registry-bootstrap-and-operations.md         |
| ADR-0073 | Composed-service SSO via Keycloak OIDC                       | 2026-06-14 | 0073-composed-service-sso-via-keycloak-oidc.md                |
| ADR-0074 | Browser-side diagnostics via Grafana Faro                    | 2026-06-14 | 0074-browser-diagnostics-grafana-faro.md                      |
| ADR-0075 | UI/UX E2E contract and future-change policy                  | 2026-06-14 | 0075-ui-ux-e2e-contract-and-future-change-policy.md           |
| ADR-0076 | Typed composition-root configuration foundation              | 2026-06-19 | 0076-typed-composition-root-configuration-foundation.md       |
...
- ADR-0001 through ADR-0052, plus ADR-0053/0054/0055/0056/0058/0059/0060/0061/0062/0063/0064/0065/0066/0067/0068/0069/0070/0071/0072/0073/0074/0075/0076, are **Accepted**; only ADR-0057 (re-scoped to billing, Phase 9 — hardened provider decision, undelivered) remains **Proposed**
- Next ADR: **ADR-0077**
