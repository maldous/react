...
| ADR-ACT-0157 | ADR-0029, ADR-0030 | Brokered third-party IdP login via a mock upstream OIDC provider — preserves the production React → BFF → Keycloak broker → upstream → callback → session flow. Delivered: in-repo mock-oidc fixture (node-oidc-provider; mock-google/azure/apple; scenarios verified/unverified/denied/provider-error/disabled; signed id_tokens with dynamic nonce echo); idempotent runtime Keycloak broker seed (KeycloakRealmAdminAdapter.upsertIdentityProvider, npm run seed:idps — no terraform); env-aware provider mode (mock/real/disabled) with a fail-fast guardrail so mock is never silently enabled in staging/prod; GET /api/auth/providers; /auth/login?provider= mapped to a validated kc_idp_hint (no injection); React /login selector. Verified live (dev): seed registered all three IdPs (idempotent); Keycloak honoured kc_idp_hint=mock-google and redirected through the broker to the mock-oidc picker; /api/auth/providers exposes no secrets; provider=evil → 400. SAML broker login stays separate and deferred (not implemented). Remaining for Done: run npm run test:e2e:identity green in CI. | Implementation | In Progress | Medium   | ADR-0029, ADR-0030                   | Architecture owner / technical lead | Done when test:e2e:identity is green in CI | services/mock-oidc/\*\*; apps/platform-api/src/server/auth-providers.ts; apps/platform-api/scripts/seed-idps.ts; apps/platform-api/src/server/auth.ts,routes.ts,http.ts; packages/adapters-keycloak/src/index.ts; apps/react-enterprise-app/src/routes/login.tsx, src/auth/login-providers.ts, src/msw; e2e/identity/broker-login.spec.ts; docs/local-development/mock-identity.md; docs/evidence/infrastructure/mock-oidc-broker-tests.md |
...
| ADR-ACT-0202 | ADR-0035           | Alloy log-label policy fix (found during ADR-ACT-0194/0196, verifying against live Loki). `docker/alloy/config.alloy` promotes high-cardinality Pino fields (requestId, durationMs, path, status, operationName, method) to Loki **labels**, violating the ADR-0035 label policy — these must remain log content (JSON fields / structured metadata queried at read time), not indexed labels, to avoid index bloat and memory pressure. Fix: restrict the Alloy label set to low-cardinality `service`/`environment`/`level` and rely on JSON field filtering for the rest (the log search adapter already queries them that way, so no API change is needed). Separately, the test environment does not ingest platform-api logs into Loki (only grafana/pgadmin appear in react-test Loki) while prod ingests correctly — investigate react-test Alloy container discovery. Verify both with `scripts/smoke/loki-smoke.sh test` and a label-cardinality check on the shared instance.                                                                                                                         | Implementation | Open        | Medium   | ADR-ACT-0191, ADR-ACT-0194           | Architecture owner / technical lead | Before production hardening                | docker/alloy/config.alloy; scripts/smoke/loki-smoke.sh                                                                                                                                                                                                                                                                                                                                                                                     |
