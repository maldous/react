...
| ADR-ACT-0187 | ADR-0029, ADR-0030 | Explicit system-admin support mode. system-admin cross-tenant access is currently blocked at the pipeline FQDN cross-check (no silent bypass). Future support access (viewing a tenant's data while on their subdomain) requires an explicit audited support session: actor.globalRole=system-admin, effectiveTenantId=tenant1, supportAccessReason required, audit emitted before access. Until this is implemented, system-admin must operate from the global host only. | Implementation | Open | High | ADR-0029, ADR-0030 | Architecture owner / technical lead | Before production support tooling | apps/platform-api/src/server/pipeline.ts |
