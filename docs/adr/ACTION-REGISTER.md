...
| ADR-ACT-0186 | ADR-0030 | Per-tenant Keycloak realm admin service account. Auth Settings routes currently use the global platform-provisioner credential (KEYCLOAK_PROVISIONER_CLIENT_ID). This credential is realm-management scoped per-realm (not master) so it cannot manage other realms. A dedicated per-tenant service account stored in tenant secret storage (not an env var) is the target model per ADR-0030 §5. Requires: secret storage design, per-realm service account creation during provisioning, route handler to resolve credential from storage. | Implementation | Open | Medium | ADR-0030, ADR-ACT-0185 | Architecture owner / technical lead | Before audit compliance review | apps/platform-api/src/server/routes.ts (auth settings handlers) |
