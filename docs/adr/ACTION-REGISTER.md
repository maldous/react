...
| ADR-ACT-0081 | ADR-0012 | Upgrade validate-source-imports to full TypeScript module resolution using ts.resolveModuleName or TypeScript Program/CompilerHost. Load tsconfig.json if present; resolve path aliases and barrel re-exports; validate public API entry points per package exports field; fail on unresolved bare specifiers in strict mode; report unresolved imports with specific resolution paths for actionable error messages. | Tooling | Done | Medium | ADR-ACT-0080 | Architecture owner / technical lead | Complete | Wired buildPackageMap into index replacing discoverKnownPackages; added no-architecture-in-product universal rule; fixture + test entry + reporter updated; v0.2.0. Full resolver deferred to ADR-ACT-0082 (no tsconfig.json in repo yet). |
| ADR-ACT-0082 | ADR-0012 | Full TypeScript compiler module resolution for validate-source-imports: tsconfig-loader, module-resolver with path-alias support, TypeScript Program/CompilerHost. Prerequisite: tsconfig.json files must exist in apps/packages. Supersedes the resolver portion of ADR-ACT-0081. | Tooling | Open | Medium | ADR-ACT-0081 | Architecture owner / technical lead | When tsconfig.json infrastructure exists |  |
