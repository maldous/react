...
| ADR-ACT-0182 | ADR-0029, ADR-0030, ADR-0031 | pgAdmin RLS-aware database access model. pgAdmin is an admin tool with two pre-configured PostgreSQL server connections: (1) pgadmin_sysadmin — non-superuser, app.bypass_rls=true default, all-tenant read-write, for system-admin; (2) pgadmin_tenant_admin — non-superuser, RLS fully enforced, SELECT-only, for tenant-admin. Both roles are created by docker/postgres/init-extra-databases.sh. Caddy forward_auth gates /pgadmin/\* to system-admin AND tenant-admin (admin:pgadmin added to both SYSTEM_ADMIN_RESOURCES and TENANT_ADMIN_RESOURCES). Tenant-admins must set app.current_tenant_id manually in the pgAdmin query tool before running tenant-scoped queries. Keycloak SSO via OAuth2 (public PKCE) handles authentication — no separate pgAdmin login after Keycloak session is active. | Implementation | Done | High | ADR-ACT-0155, ADR-ACT-0179 | Architecture owner / technical lead | Complete | docker/pgadmin/config_local.py; docker/pgadmin/servers.json; docker/postgres/init-extra-databases.sh; forward-auth.ts TENANT_ADMIN_RESOURCES; compose.yaml pgadmin service |
