...
SonarQube is required locally as a pre-slice hard gate using a custom "Governance Tooling" quality gate (bugs=0, vulnerabilities=0, security_hotspots_reviewed=100%, reliability_rating=A, security_rating=A, maintainability_rating=A, code_smells=0). Coverage and duplication thresholds are not enforced because: (1) architecture tooling uses `node --test` without LCOV generation, and (2) similar argument-parsing patterns across tools are intentional and architecturally consistent. Coverage tooling is tracked separately; the coverage condition is deliberately omitted because frontend (vitest) coverage is not ingested into Sonar, so an 80% new-code coverage gate would structurally fail any change touching the SPA. This gate is created and assigned idempotently by `scripts/sonar/ensure-quality-gate.sh` (invoked by `make sonar`), so it is restored after a fresh sonar-postgres volume rather than falling back to the coverage-enforcing built-in "Sonar way" gate. Sonar runs as a **test-stage** promotion gate (`scripts/stages/run-stage.sh` step 9), not in CI — see ADR-ACT-0092 (Done, 2026-06-09): CI runs the architecture orchestrator and the Tier-2 hard gates; SonarQube stays in the test environment by design.
