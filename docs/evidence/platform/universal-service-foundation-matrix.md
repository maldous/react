...
<!-- Generated from docs/evidence/platform/universal-service-foundation-registry.json on 2026-06-13. 54 capability rows. -->
...
| `api-keys-pat` | missing | Programmatic credentials for users and service accounts. | none | build | Keycloak service accounts / client credentials + custom token store | per-environment | per-env | P1 | L | High |
...
| `entitlements` | missing | Resolve what a tenant/plan is entitled to use; gate features by entitlement, not just flags. | feature flags exist but are not plan-linked entitlements | build | custom entitlement table + OpenMeter/Lago plan link | per-environment | per-env | P1 | L | High |
...
| `api-keys-pat` | key scoped to tenant + actor | hashed secret at rest; RLS | tenant.apikeys.* (to define) | key issue/revoke audit (to build) | n/a | missing | missing | missing | not-yet-proven | full capability absent | ADR-0065 | ADR-ACT-0250 | Secret handling must reuse write-only + redaction pattern from auth credentials. |
...
| `abac-pdp` | per-resource policy in realm | policy stored in Keycloak realm | resource + umaScope on routes (pipeline PEP) | policy change via resource-policies usecase | invariant-ready | /api/auth/settings/resource-policies | partial (auth settings) | n/a | authorize-resource unit tests | no general ABAC attribute model beyond UMA scopes; entitlements not modelled | ADR-0058 | ADR-ACT-0242 | Decision: keep Keycloak UMA as PDP; do NOT add OPA unless an attribute-policy need is proven. |
...
| `entitlements` | tenant-scoped entitlement set | RLS | tenant.entitlements.read | entitlement change audit (to build) | to define | missing | missing | missing | not-yet-proven | no entitlement model; tightly coupled to billing decision | ADR-0057 | ADR-ACT-0241 | Foundation dependency for billing/quotas; sequence before billing UI. |
...
| `metering-usage-meters` | missing | Ingest usage events and aggregate into meters. | clickhouse available as sink; no meter pipeline | compose | OpenMeter (OSS, ClickHouse-backed) | per-environment | per-env | P1 | L | High |
| `quota-enforcement` | missing | Enforce per-tenant limits derived from plan/entitlement. | none (feature flags only) | build | built-in quota check using entitlements + Redis counters | per-environment | per-env | P1 | M | Medium |
...
| `metering-usage-meters` | tenant-tagged meter events | ClickHouse partition by tenant | tenant.metering.read | n/a (events are the data) | to define | missing | missing | missing | not-yet-proven | no ingestion pipeline; ClickHouse is per-env analytics, not yet a meter store | ADR-0061, ADR-0057 | ADR-ACT-0245 | OpenMeter reuses ClickHouse (already composed) — strong local-first fit. |
| `quota-enforcement` | tenant-scoped counters | Redis + Postgres | enforced at BFF | quota-exceeded events | to define | missing | missing | missing | not-yet-proven | depends on entitlement engine | ADR-0057 | ADR-ACT-0241 | Blocked behind entitlements + metering. |
...
| Capability | Status | Purpose | Compose / provider | Decision | Local free candidate | Environment model | Shared/per-env | Priority | Size | Risk |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `event-bus-queues-dlq` | partial | Internal eventing, durable job queues, dead-letter + redrive. | redis; queue-runtime port scaffold; webhook-delivery worker real | build | Redis Streams (composed) or NATS/Redpanda if scale demands | per-environment | per-env (Sentry Kafka is Sentry-only, not the platform bus) | P1 | L | Medium |
...
| Capability | Tenant isolation | Data isolation | Permission | Audit events | Readiness model | BFF contract | Admin UI | Self-service UI | Proof | Production blockers | ADR | ADR-ACT | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `event-bus-queues-dlq` | tenant-tagged events | queue keys tenant-scoped | internal + tenant.webhooks.* | webhook deliveries + redrive audited | tenant-webhooks readiness kind | /api/org/webhooks* (outbound); no internal-bus API | /admin/webhooks (delivered) | tenant-admin webhooks | proof:webhooks; proof:webhook-redrive; proof:webhook-worker | outbound webhooks delivered; general internal event bus / pub-sub topics absent | ADR-0059, ADR-0051, ADR-0052 | ADR-ACT-0243 | Webhook substrate is real and proven; generalise to internal eventing carefully. |
...
| Capability | Status | Purpose | Compose / provider | Decision | Local free candidate | Environment model | Shared/per-env | Priority | Size | Risk |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `background-workers-runtime` | partial | Run scheduled and async work reliably with retries. | webhook-delivery worker (real); worker-runtime port scaffold | build | built-in worker-runtime + queue (composed) | per-environment | per-env | P2 | M | Medium |
...
| Capability | Tenant isolation | Data isolation | Permission | Audit events | Readiness model | BFF contract | Admin UI | Self-service UI | Proof | Production blockers | ADR | ADR-ACT | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `background-workers-runtime` | jobs carry tenant context | job payload tenant-scoped | internal | worker heartbeat (in-memory) | worker status (running/idle/stopped/error) | platform services readiness exposes worker status | /admin/platform (worker status) | n/a | proof:platform-services; proof:webhook-worker | single worker; in-memory heartbeat (no persistence); no general job runner or scaling | ADR-0059, ADR-0055 | ADR-ACT-0243 | Heartbeat persistence deferred per operations-cockpit evidence. |
...
| `api-docs-portal-sdk-ratelimits` | scoped API keys (depends on api-keys-pat) | n/a (docs) / per-tenant keys | tenant.developer.* | key + app changes audited | to define | openapi.json (drift not enforced); GraphQL | missing (portal) | missing | openapi:drift (not complete) | no portal, SDK gen, rate limits, sandbox/test mode, or OpenAPI explorer; OpenAPI drift incomplete | ADR-0065 | ADR-ACT-0250 | OpenAPI baseline exists; portal/SDK/rate-limit are gaps. |
...
| `service-catalog-provider-model` | host authority in catalog | no secrets exposed | tenant.platform.read | n/a | service readiness | platform services readiness | /admin/platform | n/a | proof:platform-services; proof:service-clickthrough-policy | no formal provider-adapter abstraction beyond hexagonal ports | ADR-0055 | ADR-ACT-0239 | Strong existing base; generalise to a provider catalog. |
