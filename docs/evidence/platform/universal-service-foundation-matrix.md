...
<!-- Generated from docs/evidence/platform/universal-service-foundation-registry.json on 2026-06-13. 59 capability rows. -->
...
| `end-user-profile-self-service` | locally proven | End users manage their own profile, preferences, notification settings, sessions, and devices. | postgres (user_profiles, RLS) | build | Postgres-backed user_profiles (built-in) | per-environment | per-env | P1 | L | Medium |
...
| `end-user-profile-self-service` | profile tenant + user scoped (RLS); own-profile-only (session userId) | RLS; userId is the session subject, never a param; no secrets | profile.read_self / profile.update_self (own user) | profile.updated (audit-before-change) | n/a (always-on once built) | /api/me/profile | /admin/account | /admin/account (own profile + preferences) | proof:profile-self-service; proof:notification-dispatch | in-app inbox UI is a later surface | ADR-0068 | ADR-ACT-0260 | Phase 6 delivered + live-proven: own-profile-only self-service (userId is the session subject), validated + audited, RLS tenant isolation. |
...
| `workflow-engine-scheduled-jobs` | tenant-scoped workflow namespaces | engine DB per-env | platform.workflow.* / tenant.workflow.read | workflow transitions audited | to define (engine health) | missing | missing (workflow visibility) | missing | not-yet-proven | no engine, scheduler, approval workflow, or visibility | ADR-0059 | ADR-ACT-0262 | Scheduled jobs delivered + live-proven as scheduled-jobs-builtin (Phase 5.5, on the Phase-5 outbox). The workflow engine (Windmill/Temporal) + cron expressions + a running scheduler daemon remain deferred. |
| `scheduled-jobs-builtin` | jobs tenant-scoped (RLS); each enqueued event preserves organisation_id | RLS; no secret payload fields | platform.jobs.read/write (operator-only) | scheduled_job.set + scheduled_job.run (audit-before-change) | n/a (runDueJobs tick invocable; daemon loop not yet wired) | /api/admin/scheduled-jobs; /api/admin/scheduled-jobs/:jobId/run | /admin/scheduled-jobs | n/a (operator-only) | proof:scheduled-jobs; proof:scheduled-job-routes | a running scheduler daemon (tick is invocable, not looped); cron expressions (fixed interval only) | ADR-0059 | ADR-ACT-0262 | Phase 5.5 delivered + live-proven: scheduled jobs on the proven Phase-5 outbox, idempotent per due window, paused jobs do not enqueue. Workflow engine remains a later decision. |
| `notifications` | preferences + log tenant + user scoped (RLS) | RLS; no secret payload fields (rejected at dispatch) | profile.read_self/update_self (own prefs); platform.notifications.read/write (operator) | notification.preferences_changed + notification.tested (audit-before-change) | operator notification readiness (local channels; never faked) | /api/me/notification-preferences; /api/admin/notifications/readiness; /api/admin/tenants/:tenantId/notifications/test | /admin/account | /admin/account (preference toggles) | proof:notification-preferences; proof:notification-dispatch | real delivery transports (Mailpit/Brevo/webhook-POST) + composed provider (Novu) are Phase 6.5 | ADR-0068 | ADR-ACT-0260 | Phase 6 delivered + live-proven: preference-gated dispatch (disabled suppresses), durable log, local channels, no paid provider. Real transports + Novu are Phase 6.5 behind NotificationDispatchPort. |
...
| `runtime-secrets` | locally proven | Central, audited secret storage and rotation. | built-in Postgres secret_refs (migration 031, RLS, AES-256-GCM at rest); composed OpenBao (secrets profile, KV v2) — both live-proven; LocalStack secretsmanager remains MOCK only | build | built-in Postgres secret store (default) + OpenBao (open-source, Vault-compatible) behind SecretStorePort | per-environment | per-env | P2 | M | High |
...
| `runtime-secrets` | secret_refs organisation-scoped (RLS); OpenBao KV path <base>/<org>/<ref>; opaque secret:<uuid> ref | value encrypted at rest (built-in) or held in OpenBao (encrypted_value NULL); plaintext never returned by any read/list/readiness; resolve() server-internal only | platform.secrets.read / platform.secrets.write (operator-only) | secret_ref.created / secret_ref.revoked / secret_ref.deleted (audit-before-change; name/ref only, never the value) | operator secret-store readiness (built-in Postgres SELECT 1 / OpenBao sys/health; never faked — OpenBao down ⇒ degraded) | GET/POST /api/admin/secrets; POST /api/admin/secrets/revoke\|delete; GET /api/admin/secrets/readiness | n/a (operator API; no UI surface in this pass) | n/a (operator-only) | proof:secret-store-contract; proof:secrets-openbao; proof:provider-secrets-readiness | OpenBao -dev mode is DEV/TEST only — production requires sealed/HA/auto-unseal + externalised credentials; rotation automation + dynamic/leased secrets not delivered; OpenBao UI is not_exposed (operators use the direct port) | ADR-0069, ADR-0055, ADR-0031 | ADR-ACT-0265 | Tier-1 kernel (ADR-ACT-0265): central secret store behind SecretStorePort — built-in PostgresSecretStore (durable default, encrypted at rest) + composed OpenBaoSecretStore (value in OpenBao KV v2 over fetch, no new dependency), both live-proven. Opaque secret:<uuid> ref; resolve() server-internal only. LocalStack secretsmanager remains mock-only. Production OpenBao topology + rotation are Proposed sub-decisions (not delivered). |
...
| `alerting-incident-oncall` | partial | Alert rules, incident lifecycle, escalation, and public status. | grafana exists but no Alertmanager; no incident tool | compose | Grafana Alerting + Alertmanager (OSS); status page via custom + readiness API | shared-cross-environment | shared (engineering ops) with env labels; tenant comms separate | P1 | L | High |
| `observability-alerting-builtin` | locally proven | Built-in metric-signal registry, threshold alert rules + evaluation, incident lifecycle, and an alert→notification bridge. | postgres (metric_signals/metric_samples/alert_rules/incidents, RLS) | build | built-in Postgres signal/alert/incident store (no external backend) | per-environment | per-env | P1 | L | Medium |
...
| `metrics-traces` | tenant/env labels on telemetry | label-based | operator | n/a | observability readiness signals (metrics/traces status) | observability readiness route | /admin/observability (signal status only) | n/a | observability-smoke; proof:tenant-observability | OTEL collector ingests but no metrics/trace backend or dashboards | ADR-0062, ADR-0020 | ADR-ACT-0261 | Built-in metric-signal registry + sample store delivered + live-proven (observability-alerting-builtin); a real Prometheus/Tempo backend remains Phase 7.5 behind MetricRepository. |
| `alerting-incident-oncall` | alerts tagged by env/tenant; tenant comms scoped | label-based; access-controlled | platform.ops.* | incident actions audited | service readiness already exists internally | platform services readiness (internal); no incident API | /admin/platform (readiness); no incident UI | missing (public status page) | proof:platform-services (readiness only) | no alert rules, notification channels, SLOs, incident lifecycle, on-call, or status page | ADR-0062 | ADR-ACT-0261 | Built-in alerting + incident lifecycle + alert→notification bridge delivered + proven (observability-alerting-builtin). Composed Alertmanager, on-call/escalation, and the public status page remain Phase-7.5 (not delivered). |
| `observability-alerting-builtin` | signals/samples/rules/incidents tenant-scoped (RLS) | RLS; no secret fields; alert payloads carry no secrets | platform.observability.read/write (operator-only) | alert.rule_set + incident.opened + incident.updated (audit-before-change) | operator observability readiness (built-in; never faked) | /api/admin/observability/signals; /api/admin/alerts; /api/admin/incidents; /api/admin/observability/readiness | /admin/monitoring | n/a (operator-only) | proof:observability-signals; proof:alerting; proof:incident-foundation; proof:alert-notification-bridge | composed metrics/trace + alerting backends (Phase 7.5); on-call/escalation; status page | ADR-0062 | ADR-ACT-0261 | Phase 7 delivered + live-proven: built-in alerting + incident lifecycle + alert→notification bridge (Phase-6 substrate, preference-gated). Composed Prometheus/Tempo/Alertmanager + on-call + status page are Phase 7.5 behind the ports. |
...
| `rate-limiting` | locally proven | Per-tenant request rate limits over a fixed window, entitlement-gated. | postgres (rate_limit_policies + rate_limit_counters, RLS) default; redis (per-env) is the Phase-3.5 counter provider behind the port | build | Postgres fixed-window counter (built-in, default); Redis counter provider (RATE_LIMIT_PROVIDER=redis) delivered behind the port — Redis is a default Compose service | per-environment | per-env | P1 | M | Medium |
...
| `rate-limiting` | policies + counters tenant-scoped (RLS) | RLS; no secret fields | tenant.developer.read; platform.rate_limits.read/write | rate_limit.set (audit-before-change) | n/a (always-on once built) | /api/org/rate-limits; /api/admin/tenants/:tenantId/rate-limits | /admin/developer | tenant read-only | proof:rate-limits; proof:rate-limits-redis; proof:api-key-routes | Redis counter provider delivered + live-proven behind the port (Phase 3.5); policy definitions stay durable in Postgres; the limiter degrades to the Postgres fallback when Redis is unavailable (never fails open) | ADR-0065 | ADR-ACT-0257, ADR-ACT-0263 | Phase 3 delivered + live-proven (Postgres default). Phase 3.5 (ADR-ACT-0263): Redis counter provider behind RateLimitRepository (RATE_LIMIT_PROVIDER=redis) — atomic Lua INCR+EXPIRE, tenant-prefixed keys, no secrets, self-expiring TTL, honest Postgres fallback + degraded readiness; live-proven (proof:rate-limits-redis). Reuses the entitlement substrate (entitlement before limit), the bridge to the quota model (ADR-0067). |
...
| `provider-configuration` | locally proven | Bind a USF capability to a concrete provider per environment, with classification, lifecycle, and credentials by secretRef. | built-in Postgres provider_configs (migration 032); operator-global infra | build | built-in Postgres provider-config plane behind ProviderConfigRepository | per-environment | per-env | P1 | M | Medium |
| `composed-provider-readiness` | locally proven | Live, port-backed readiness probe per composed provider feeding the adapter-confirmed provider-config lifecycle. | search-provider (Meilisearch) + observability-provider (Prometheus/Tempo/Alertmanager) profiles | compose | generic HttpProviderReadinessProbe (fetch) behind ProviderReadinessProbe; light providers composed locally | per-environment | per-env | P1 | M | Medium |
...
| `provider-configuration` | operator-global infra (no tenant data); accessed via withSystemAdmin | no plaintext secret stored (credential_ref is an opaque secret:<uuid>, DB CHECK); config rejects secret-bearing keys | platform.providers.read / platform.providers.write (operator-only) | provider_config.set / provider_config.deleted (audit-before-change; no secret) | adapter-confirmed lifecycle (deriveReadinessLifecycle) — config alone never implies ready | GET/POST /api/admin/provider-configs; POST /api/admin/provider-configs/:id/lifecycle; POST /api/admin/provider-configs/:id/delete | n/a (operator API; no UI surface in this pass) | n/a (operator-only) | proof:provider-config; proof:provider-readiness-contract | per-tenant provider overrides + automatic adapter-driven lifecycle reconciliation not delivered (operator-set + on-demand derivation) | ADR-0070 | ADR-ACT-0266 | Tier-1 kernel (ADR-ACT-0266): operator-global, environment-scoped provider config plane behind ProviderConfigRepository. Credentials by secretRef into the ADR-0069 store (never plaintext); forbidden-in-production cannot be active in prod; missing-credential is degraded; ready is adapter-confirmed; a candidate never implies a delivered capability. Live-proven (proof:provider-config + proof:provider-readiness-contract). |
| `composed-provider-readiness` | readiness probe holds no tenant data (operator-global health check) | no tenant data; no secret in the readiness payload (auth header sent, never echoed) | platform.providers.read (operator-only) | n/a (read-only health probe) | live HTTP health probe; ready only on a 2xx; degraded when wired-but-unreachable; not_configured when no endpoint; lifecycle adapter-confirmed (deriveReadinessLifecycle) | GET /api/admin/providers/readiness | n/a (operator API; no UI surface in this pass) | n/a (operator-only) | proof:composed-provider-readiness | readiness != backend integration — Meilisearch/Prometheus/Tempo are NOT the active backend (built-in substrates remain default); Windmill/Temporal probe-ready but not composed; local provider profiles are dev configs | ADR-0071 | ADR-ACT-0271 | Composed-provider pass (ADR-ACT-0271): generic readiness probe + compose profiles for the light providers. Meilisearch/Prometheus/Alertmanager live readiness-proven; Windmill/Temporal not_configured (no default endpoint). Backend integration per provider is a deferred sub-decision; the built-in substrates (ADR-0060/0062/0059) stay default. |
