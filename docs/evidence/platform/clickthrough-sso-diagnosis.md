...
| **sonarqube** | OIDC auto-login; default admin rotated to the managed password | "default administrator credentials still used"; live: `admin:admin` validated `true` even with OIDC enabled | **Code defect**: `provision-token.sh` `exit 0` (§3) ran *before* the password rotation (§4b); once the token was minted the rotation never ran → `admin:admin` persisted | **Fixed** — reorder (rotation always runs) + `make sonar-provision` |
