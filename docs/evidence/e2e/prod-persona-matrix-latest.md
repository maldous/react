...
- ✅ [unauthenticated-visitor] expected-route `/` → expected loads (non-blank), got loads
- ✅ [unauthenticated-visitor] expected-route `/login` → expected loads (non-blank), got loads
...
- ✅ [scaffold-system-admin] expected-route `/` → expected loads (non-blank), got loads
...
- ✅ [scaffold-system-admin] clickthrough-granted `keycloak /kc/` → expected service UI (status<400, not SPA), got status=200 url=<https://aldous.info/kc/realms/master/protocol/openid-connect/auth?client_id=security-admin-console&redirect_uri=https%3A%2F%2Faldous.info%2Fkc%2Fadmin%2Fmaster%2Fconsole%2F&state=93f69c24-6ef2-49d2-8e2e-cffbe1101938&response_mode=query&response_type=code&scope=openid&nonce=358b1d1a-c023-46a8-885c-edc94dc7e6ed&code_challenge=hf_TmCxUZcBmBOkErqAJ4EuXCow1uCiKQ01DjClop80&code_challenge_method=S256>
...
- ✅ [scaffold-tenant-admin] expected-route `/` → expected loads (non-blank), got loads
- ✅ [scaffold-tenant-admin] expected-route `/admin` → expected loads (non-blank), got loads
- ✅ [scaffold-tenant-admin] expected-route `/admin/members` → expected loads (non-blank), got loads
...
- ✅ [scaffold-tenant-manager] expected-route `/` → expected loads (non-blank), got loads
...
- ✅ [scaffold-tenant-member] expected-route `/` → expected loads (non-blank), got loads
- ✅ [scaffold-tenant-member] expected-route `/admin` → expected loads (non-blank), got loads
...
- ✅ [scaffold-support-breakglass] expected-route `/admin` → expected loads (non-blank), got loads
