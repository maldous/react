...
const script = path.join(repoRoot, "tools", "architecture", "validate-package-metadata", "src", "index.mjs");
const packageJsonPath = path.join(repoRoot, "tools", "architecture", "validate-package-metadata", "package.json");

function run(args) {
  return spawnSync(process.execPath, [script, "--root", repoRoot, "--allow-missing-ajv", "--no-reports", "--format", "json", ...args], {
    cwd: repoRoot,
    encoding: "utf8"
  });
}
...
const strictMissingAjv = spawnSync(process.execPath, [script, "--root", repoRoot, "--no-reports", "--format", "json", "tools/architecture/validate-package-metadata/tests/fixtures/valid"], {
  cwd: repoRoot,
  encoding: "utf8"
});
...
const missing = run(["tools/architecture/validate-package-metadata/tests/fixtures/invalid/missing-architecture"]);
...
const invalidEnum = run(["tools/architecture/validate-package-metadata/tests/fixtures/invalid/invalid-enum"]);
assert.equal(invalidEnum.status, 1, invalidEnum.stderr || invalidEnum.stdout);
assert.equal(parse(invalidEnum).failed, 1);

const semantic = run(["tools/architecture/validate-package-metadata/tests/fixtures/invalid/semantic-lifecycle-class"]);
...
const reportResult = spawnSync(process.execPath, [script, "--root", repoRoot, "--allow-missing-ajv", "tools/architecture/validate-package-metadata"], {
  cwd: repoRoot,
  encoding: "utf8"
});
...
assert.ok(fs.existsSync(path.join(repoRoot, "reports", "validation", "package-metadata-validation.json")));
assert.ok(fs.existsSync(path.join(repoRoot, "reports", "validation", "package-metadata-validation.md")));
