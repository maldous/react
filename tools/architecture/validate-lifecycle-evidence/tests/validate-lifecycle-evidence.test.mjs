...
test("validate-lifecycle-evidence check, coverage, generation, and self-evidence", async () => {
  ...
  const goldenLifecycleEvidenceRoot = path.join(
    repoRoot,
    "tools",
    "architecture",
    "validate-lifecycle-evidence",
  ...
  function run(root, args) {
    return spawnSync(
      process.execPath,
      [script, "--root", root, "--allow-missing-ajv", "--no-reports", "--format", "json", ...args],
      {
        cwd: repoRoot,
        encoding: "utf8",
        env: {
  ...
  for (const [fixtureName, expectedErrorFragment] of Object.entries(
    invalidTransitionExpectations
  )) {
    ...
    assert.match(
      errorText,
      new RegExp(expectedErrorFragment.replace(".", "\\.")),
      `${fixtureName} error should mention ${expectedErrorFragment}: ${errorText}`
    );
  }
  ...
  const generated = run(sourceRoot, [
    ...
    "stable.platform",
    "--reason",
    "Ready for stable platform promotion",
  ]);
  ...
  const generatedBundle = JSON.parse(
    fs.readFileSync(path.join(sourceRoot, generatedPayload.outputPaths[0]), "utf8")
  );
  ...
  assert.equal(
    generatedBundle.sourceMetadataSnapshot.packageJsonPath,
    "packages/package-a/package.json"
  );

  assertGeneratedFileEquals(
    sourceRoot,
    goldenLifecycleEvidenceRoot,
    generatedPayload.outputPaths[0]
  );
  assertGeneratedFileEquals(
    sourceRoot,
    goldenLifecycleEvidenceRoot,
    generatedPayload.outputPaths[1]
  );

  const evidence = spawnSync(
    process.execPath,
    [script, "--root", validRoot, "--allow-missing-ajv", "--format", "json"],
    {
      cwd: repoRoot,
      encoding: "utf8",
    }
  ...
  assert.match(
    ...
    /^reports\/tooling\/validate-lifecycle-evidence\/.+-run\.json$/
  );
